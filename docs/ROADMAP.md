...
- v0.4.0の「シングルタップで生成／ダブルタップで終了」はPhase 1の検証用操作。Phase 2ではパッシブ通知／選択式深掘りへ移行し、音声入力はSDK標準機能が利用可能になった時点で再評価する
...
- Issue #31で外部音声認識に依存しない3候補の選択式深掘りをv0.9.0として導入し、実機フィードバックによる操作修正版をv0.9.1として実装中。候補中は自動消灯を停止し、上下で選択、シングルタップで決定、ダブルタップで直前表示へ戻る。終了はEven共通の長押し確認に委ねる
...
## Phase 2 — 文脈と操作
**完了条件**：移動・滞在時間を加味した通知をパッシブに提示し、シングルタップから選択式で深掘りし、後方／下スライドの別視点、前方／上スライドの飛ばす／閉じる、5秒の自動消灯をG2で行える。音声入力はSDK標準機能が利用可能になった時点で再評価する。
...
7. SDKの操作・音声認識機能を再確認し、利用可能になった機能だけを追加する
...
6. [#31 選択式の深掘りメニュー](https://github.com/takanakafumi/world-whisper-g2/issues/31)
