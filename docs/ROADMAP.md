...
- Issue #19でスマホの通知／消去ボタンと、G2の深掘り／別視点／閉じる状態遷移をv0.5.0として実装中
...
## Phase 2 — 文脈と操作
**完了条件**：移動・滞在時間を加味した通知をパッシブに提示し、シングルタップの深掘り、後方／下スライドの閉じる、前方スライドの別視点、5秒の自動消灯をG2で行える。長押しは終了／スリープ、ダブルタップは音声入力へ割り当てる。

### Phase 2開発順序

1. スマホ画面の「通知を発生」でパッシブ通知を代替する
2. スマホ画面の「表示を消す」で5秒自動消灯を代替する
3. G2の深掘り、閉じる、別視点と表示状態遷移を実機検証する
4. 状態遷移の安定後に、移動・滞在の自動検知を通知へ接続する
5. 最後に5秒タイマーを接続し、本番UXとして通しで検証する
